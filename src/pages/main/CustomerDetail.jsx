...
function CustomerDetail() {
  ...
  if (loading) return <Loading message="Memuat data customer..." />;
  if (error) return <div style={styles.error}>Gagal memuat: {error}</div>;
  if (!customer) return null;

  const meta = getMembershipMeta(customer.membership);
  ...
  return (
    <>
      <Link to="/customers" style={styles.backLink}>
        <FiArrowLeft /> Kembali ke daftar customer
      </Link>

      <div style={styles.header}>
        <img
          src={customer.image || `https://ui-avatars.com/api/?name=${encodeURIComponent(customer.name || 'U')}&background=DFE9F4&color=054C73&size=128`}
          alt={customer.name}
          style={styles.avatar}
        />
        <div style={{ flex: 1, minWidth: 0 }}>
          <h1 style={styles.name}>{customer.name}</h1>
          <p style={styles.username}>{customer.email}</p>
          <div style={styles.badges}>
            <span style={{ ...styles.badge, background: meta.bg, color: meta.color }}>
              {meta.label} Member
            </span>
            {customer.gender && <span style={styles.badge}>{customer.gender}</span>}
          </div>
        </div>
      </div>
      ...
      <div style={styles.grid}>
        ...
        <Card style={styles.cardOverride}>
          ...
          <InfoRow icon={<FiCreditCard />} label="Total Transaksi" value={formatRupiah(customer.total_spent)} />
        ...
        <Card style={styles.cardOverride}>
          ...
          <InfoRow icon={<FiUser />} label="Umur" value={customer.age ? `${customer.age} tahun` : '-'} />
        </Card>
      </div>
    </>
  );
